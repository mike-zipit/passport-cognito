...
function CognitoStrategy(options, verify) {
  ...
  this._clientDetailsLookup = options.clientDetailsLookup;
...
CognitoStrategy.prototype.authenticate = function(req, options) {
  ...
  this._getClientDetails(username, clientDetails => {

    var cognitoUser = this._createCognitoUser(username, clientDetails);
    ...
    cognitoUser.authenticateUser(authenticationDetails, {
      ...
      mfaRequired: function(codeDeliveryDetails) {
        ...
        if (mfaCode) {
          return cognitoUser.sendMFACode(mfaCode, this);
        } else {
          return self.fail({ message: options.badRequestMessage || 'Multi factor authentication Required', codeDeliveryDetails: codeDeliveryDetails }, 424);
        }
      },
      newPasswordRequired: function(userAttributes, requiredAttributes) {
        ...
        if (newPassword) {
          ...
          return self.fail({ message: options.badRequestMessage || 'New Password Required', requiredAttributes: requiredAttributes }, 412);
        }
      }
    });
  });
 ...
 * @access protected
 */
CognitoStrategy.prototype._getClientDetails = function(username, cb) {
  if(this._clientDetailsLookup && this._clientName){

    var myRegexp = /https?:\/\/(.*?)(\/.*?)$/g;
    var match = myRegexp.exec(this._clientDetailsLookup);
    ...
    var signer  = new aws4.RequestSigner(opts, {
    ...
    rp(opts).then(res => {
      if(res && res.UserPoolId && res.ClientId) {
        return cb({
          UserPoolId: res.UserPoolId,
          ClientId: res.ClientId
        });
      } else {
        return this.fail({ message: 'No user found with username: ' + username }, 404);
      }
    }).catch(e => {
      console.log(e);
      return this.fail({ message: 'Something went wrong while finding userpoolId for username: ' + username }, 400);
    });
  } else if(this._userPoolId && this._clientId) {
    return cb({
      UserPoolId: this._userPoolId,
      ClientId: this._clientId
    });
  } else {
    return this.fail({ message: 'Requires either [options.clientDetailsLookup + options.clientName] or [options.userPoolId + options.clientId] to be set' }, 400);
  }
};
 ...
 * @access protected
 */
CognitoStrategy.prototype._createCognitoUser = function(username, clientDetails) {

  var poolData = {
    UserPoolId : clientDetails.UserPoolId,
