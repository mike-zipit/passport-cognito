...
function CognitoStrategy(options, verify) {
  ...
  this._userPoolLookup = options.userPoolLookup;
...
CognitoStrategy.prototype.authenticate = function(req, options) {
  ...
  this._getUserpool(username, userpoolId => {


    var cognitoUser = this._createCognitoUser(username, userpoolId);
 ...
 * @access protected
 */
CognitoStrategy.prototype._getUserpool = function(username, cb) {
  if(this._userPoolLookup){
    var opts = {
      uri: 'https://qsiepcjyg5.execute-api.eu-west-1.amazonaws.com/prod/CognitoUserPoolHandler-production',
      host: 'qsiepcjyg5.execute-api.eu-west-1.amazonaws.com',
      method: 'POST',
      json: true,
      path: '/prod/CognitoUserPoolHandler-production',
      body: JSON.stringify({
        username: username
      })
    };
    ...
    var signer  = new aws4.RequestSigner(opts, {
      accessKeyId: 'AKIAIPB6MT5MTTVI4SJA',
      secretAccessKey: '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'
    });
  ...
  } else if(this._userPoolId) {
    return cb(this._userPoolId);
  } else {
    return this.fail({ message: 'Requires either options.userPoolLookup or options.userPoolId to be set' }, 400);
  }
};
 ...
 * @access protected
 */
CognitoStrategy.prototype._createCognitoUser = function(username, userPoolId) {
